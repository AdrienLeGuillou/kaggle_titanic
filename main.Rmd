...
The pre-processing of the data is now complete. It is time for imputation.

I will train several models. Compare there accuracies and the timing spent on 
training. I will use the `caret` package to have a consistent interface among
my models. The model computation will be parallelized using the `doParallel` 
...
```
```{r}
p_prob <- predict(rf_model, train, type = "prob")
p_raw <- predict(rf_model, train, type = "raw")
...
test_p <- predict(rf_model, test, type = "raw")
...
durations <- durations %>%
  rbind(list(model = "gradient boost", 
          duration = end_time - start_time))
