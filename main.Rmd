...
dataset <- read_csv("data/test.csv") %>%
  mutate(set = "test",
         Survived = NA) %>%
  bind_rows(dataset)
```
...
The variables `Passenger_id`, `name`, `sexe`, `cabin_number`, `ticket_number`,
`ticket_class` and `embark_port` are categorical and will need to be 
transformed correctly if we want to use them for prediction.
